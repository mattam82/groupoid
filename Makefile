...
VFILES:=theories/fun_ext.v\
  ...
  theories/groupoid_with_uip/groupoid_with_uip.v\
  theories/groupoid_with_uip/eq_with_uip.v
...
install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/Groupoid/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/Groupoid/$$i;\
	done
