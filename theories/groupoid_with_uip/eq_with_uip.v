...
Section TypeEq.
  ...
  Inductive seq (A : T) : A -> A -> T :=
  | seq_refl a : seq a a.

  Lemma eq_sym (A : T) (x y : A) : seq x y -> seq y x.
  Proof.
    intros []. apply seq_refl.
  Defined.

  Lemma eq_trans (A : T) (x y z : A) : seq x y -> seq y z -> seq x z.
  Proof.
    intros []. intros a []. apply seq_refl.
  Defined.

End TypeEq.

Notation " x ≡ y " := (@seq _ x y) (at level 80).
Notation " x ≡ y " := (@seq _ x y) : type_scope.

Axiom UIP : forall T (x y:T) (e e': x ≡ y), e ≡ e'.
...
Definition apD10 {A} {B:A->Type} {f g : forall x, B x} (h:f ≡ g)
  : f == g
  := fun x => match h with seq_refl _ => seq_refl _ end.
...
: u ≡ v. 
  ...
  destruct p. simpl in q. destruct q. apply seq_refl.
Defined.

Definition path_proj1 {A : Type} (P : A -> Type) (u v : sigma P)
  (p : u ≡ v) : u.1 ≡ v.1. 
  destruct p. apply seq_refl. 
Defined. 

Notation "p ..1" := (path_proj1 p) (at level 3).

Definition path_proj2 {A : Type} (P : A -> Type) (u v : sigma P)
  (p : u ≡ v) : p..1 # u.2 ≡ v.2. 
  destruct p. apply seq_refl. 
Defined. 

Notation "p ..2" := (path_proj2 p) (at level 3).

Definition path_prod_uncurried {A B : Type} (z z' : A * B)
  (pq : (fst z ≡ fst z') * (snd z ≡ snd z')): (z ≡ z').
  destruct pq. destruct z, z'. simpl in *. destruct s, s0. apply seq_refl.
Defined.
