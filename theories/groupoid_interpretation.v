...
(* Instance FunTypeHomG : HomT1 [Type1] := {eq1 := Fun_Type_Groupoid}. *)

(* Instance nat_transHomG' : HomT2 Fun_Type_Groupoid := *)
(*   {eq2 := fun T U => nat_trans (T:=|T|g) (U:=|U|g)}. *)

(* Program Instance category_funG : CategoryP [Type1]. *)
(* Next Obligation. *)
(* Proof. *)
...
(****** this one is too long for coqc ??? ******)

(* Program Instance ActionType2 : Action (T:=Context) (fun T => [ [[T]] --> Type1]) := *)
(*   {| C := category_funS ; eqAc := λ T, nat_trans (U:=Type1) ; *)
(*      action := λ T U (σ: [T -|-> U]) (f : [ [[U]] --> Type1]), (λ x, f @ (σ @ x) ; arrow_comp _ _ _ _ _) |}. *)
...
Definition fun_eq_map' {Γ : Context} (A: Typ Γ)
