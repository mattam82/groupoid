...
Definition fun_eq {T T' U U': WeakGroupoidType} (e:T ~1 T') (e': U ~1 U') : 
  (T --> U) ---> (T' --> U') := right_comp T' [e'] ° left_comp U (adjoint e).

Definition right_comp_eq (T T' U : WeakGroupoidType) (f f':[T --> T'])
...
Definition right_comp_eq' (T T' T'' U : WeakGroupoidType) (f:[T --> T']) (f' :[T' --> T''])
...
Definition left_comp_eq (T U U' : WeakGroupoidType) (g g':[U --> U'])
...
Definition nat_assoc_inv (T U U' U'' : WeakGroupoidType) (g:[U --> U']) (g' :[U' --> U'']) (f : U'' ---> T) :
  (f ° (g' ° g)) ~1 ((f ° g') ° g).
  eapply inverse. refine (nat_assoc g g' f). Defined.

Definition left_comp_eq' (T U U' U'' : WeakGroupoidType) (g:[U --> U']) (g' :[U' --> U'']) 
...
Definition right_left_comp (T T' U U' : WeakGroupoidType) (g :[T --> T']) (f: [U --> U'])
...
Definition left_comp_id (T U : WeakGroupoidType) : left_comp T (identity U) ~2 identity _.
  exists (fun t => nat_id_R t). econstructor. intros. simpl. 
  intro. simpl. simpl_id_bi. Defined.

Definition right_comp_id (T U : WeakGroupoidType) : right_comp U (identity T) ~2 identity _.
  exists (fun t => nat_id_L t). econstructor. intros. simpl. 
  intro. simpl. simpl_id_bi. Defined.

Program Instance fun_eq2 (T T' U U' : WeakGroupoidType) (e:T ~1 T') (e': U ~1 U') : Iso_struct (fun_eq e e').
...
Definition fun_eqT (T T' U U' : WeakGroupoidType) (e:T ~1 T') (e': U ~1 U') : (T --> U) ~1 (T' --> U')
  := IsoToEquiv (fun_eq e e'; fun_eq2 _ _ _ _ _ _).

Definition fun_eq_eq (T T' U U' : WeakGroupoidType) (e e':T ~1 T') (f f': U ~1 U')
           (H : e ~2 e') (H' : f ~2 f') : fun_eq e f ~1 fun_eq e' f' :=
nat_comp' (left_comp_eq _ (Equiv_adjoint [H])) (right_comp_eq _ [H']).

Definition fun_eq_eq' (T T' T'' U U' U'' : WeakGroupoidType) (e:T ~1 T') (e':T' ~1 T'') 
           (f : U ~1 U') (f' : U' ~1 U'') : 
...
Program Definition fun_eq_map {Γ : WeakGroupoidType} (A: [Γ --> _Type]) (x y z : [Γ]) (e : x ~1 y) (e' : y ~1 z) : 
...
Program Definition fun_eq_id {Γ : WeakGroupoidType} (A: [Γ --> _Type]) (x : [Γ]) :
...
Lemma nat_trans_comp (A: WeakGroupoidType) (T U : [A --> _Type]) (α : T ~1 U)
...
Lemma nat_trans_id (A: WeakGroupoidType) (T U : [A --> _Type]) (α : T ~1 U)
  (x : [A]) :
...
Lemma nat_trans2 (A: WeakGroupoidType) (T U : [A --> _Type]) (α : T ~1 U)
...
Program Instance prod_eq1 (A: WeakGroupoidType) (T U : [A --> _Type]) (eqTU : T ~1 U)
        (t : [_Prod T]) :
...
Next Obligation.
  ...
  simpl_id_bi.
  eapply (right_simplify' (T:=([U @ z]; eq_WG (U @ z)))). 
  ...
  eapply composition. eapply inverse. apply assoc.
  apply comp; try apply identity.
  eapply (left_simplify' (T:=([U @ z]; eq_WG (U @ z)))). 
  eapply composition. eapply inverse. apply assoc.
  eapply composition. apply comp. apply identity. 
  eapply composition. eapply inverse. apply assoc.
  eapply composition. apply comp. apply identity. 
...
Program Instance prod_eq2 (A: WeakGroupoidType) (T U : [A --> _Type]) (eqTU : T ~1 U) :
...
Program Definition prod_eq (A: WeakGroupoidType) (T U : [A --> _Type]) (e:T ~1 U) : [_Prod T --> _Prod U] := (_; prod_eq2 A T U e).

Definition fun_comp T U V (f : T ---> U) (g : U ---> V) := composition (Composition := comp_fun) f g.

Program Definition prod_eq_comp' (A: WeakGroupoidType) (T U V: [A --> _Type]) 
        (e:T ~1 U) (e' : U ~1 V) : 
...
apply inverse. eapply composition. eapply inverse. eapply (comp_inv (T := ([V @ t']; eq_WG (V @ t')))). 
...
Program Definition prod_eq_map' (A: WeakGroupoidType) (T U: [A --> _Type]) 
...
Program Definition prod_eq_id' (A: WeakGroupoidType) (T: [A --> _Type]) :
...
Program Instance prod_eq_iso (A: WeakGroupoidType) (T U : [A --> _Type]) (e:T ~1 U) : 
...
Program Definition prod_eqT (A: WeakGroupoidType) (T U : [A --> _Type]) (e:T ~1 U) : 
  _Prod T <~> _Prod U := IsoToEquiv (prod_eq e; prod_eq_iso _ _ _ e).

(* Program Instance fun_todep_fun_2 (T U: WeakGroupoidType) : WeakFunctor (λ _ : [T], U). *)
...
(* Program Definition fun_todep_fun_1 (T U: WeakGroupoidType) : [T --> _Type] :=  *)
(*   (λ _ : [T], U; fun_todep_fun_2  _ _). *)

(* Program Instance fun_todep_fun1 (T U: WeakGroupoidType) (M : [T --> U]) : WeakDependentFunctor (fun_todep_fun_1 T U) [M]. *)
...
(* Program Definition fun_todep_fun (T U: WeakGroupoidType) (M : [T --> U]) :  *)
